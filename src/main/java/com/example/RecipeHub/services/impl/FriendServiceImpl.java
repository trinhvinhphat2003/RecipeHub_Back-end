...
public class FriendServiceImpl implements FriendService{
	...
	public FriendServiceImpl(UserRepository userRepository, FriendshipRequestRepository friendshipRequestRepository) {
		super();
	...
	public ArrayList<FriendshipRequestDTO> getAllFriendshipRequest(User user) {
		...
		List<FriendshipRequest> friendshipRequests = friendshipRequestRepository.findAllByReceiverAndStatus(user,
				Friendship_status.PENDING);
		for (FriendshipRequest friendshipRequest : friendshipRequests)
			friendshipRequestDTOs
					.add(FriendshipRequestMapper.INSTANCE.friendshipRequestToFriendshipRequestDTO(friendshipRequest));
	...
	public ArrayList<FriendshipRequestDTO> getAllSendedFriendshipRequest(User user) {
		user = userRepository.findById(user.getUserId()).orElseThrow(() -> new NotFoundExeption(""));
		ArrayList<FriendshipRequestDTO> friendshipRequestDTOs = new ArrayList<>();
		List<FriendshipRequest> friendshipRequests = friendshipRequestRepository.findAllBySenderAndStatus(user,
				Friendship_status.PENDING);
		for (FriendshipRequest friendshipRequest : friendshipRequests)
			friendshipRequestDTOs
					.add(FriendshipRequestMapper.INSTANCE.friendshipRequestToFriendshipRequestDTO(friendshipRequest));
